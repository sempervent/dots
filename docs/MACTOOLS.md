...
Linux / non-Darwin: explicit `--with mactools` errors (same platform gate as `fluidvoice` / `cursor`).
...
Migration is manual and reversible: `docs/ORBSTACK_MIGRATION.md`  
...
separate, intentional migration. Analysis only: `docs/MISE_MIGRATION.md`.
