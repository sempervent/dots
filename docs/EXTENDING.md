...
artifacts. Read [ARCHITECTURE.md](ARCHITECTURE.md) for sources of truth.
...
plus any focused tests named below.
...
configs/packages/xbps.toml
  ↓
tests + docs/TOOLS.md if conceptually significant
```
...
[ARCHITECTURE.md](ARCHITECTURE.md). Do not delete them casually.
...
distro-specific override only for real incompatibilities
  ↓
docs/LINUX.md + CI smoke expectations
```
...
Follow [LINUX.md](LINUX.md) and the smoke harness under `scripts/ci/distro_smoke*.sh`
plus `.github/workflows/ci.yml`. Keep smoke non-mutating where possible.
...
See package ownership terminology in the README and `./dots packages status`.

---

## Adding a tool to the documentation catalog

Add a row to [TOOLS.md](TOOLS.md) with an official upstream link. Do not list every
