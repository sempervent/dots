...
Generated inventory: [skills reference](../reference/generated/skills.md)
(from `configs/skills/manifest.toml`).
...
| Pack | Groups (from manifest) |
|------|------------------------|
| `skills` | security, core, engineering, infrastructure, testing, visualization |
| `ai-skills` | security, ai |

Do **not** install the entire magnus919 catalog — only rows listed in the
manifest with `enabled = true`.
...
4. Document in [TOOLS](tools.md) if notable.

### Skill with Homebrew dependencies

Prefer `brewfile =` on the component (shared file allowed, e.g. Archify). Apply
path is resolved by `dots_component_brewfile` — see [Extending](extending.md).
...
Focused tests: `scripts/tests/skills_location_test.sh`,
`scripts/tests/skills_desired_state_test.sh`, plus
`scripts/tests/repository_contract_test.sh` (manifest integrity).

## Related

- Maintainer operating manual: root `SKILL.md` (dots-maintainer)
- Routing policy skill: `skills/agent-router/SKILL.md` (distinct)
