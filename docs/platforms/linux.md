...
Homebrew is **not** required on Linux servers; native package managers are
preferred for the server profile smoke path.
...
It is **not** identical to booting Raspberry Pi OS on Pi hardware. See
...
Use the shared `pacman.toml` map. Prefer `base` or `server` on headless boxes;
`home` on desktop if you want GUI groups (many GUI tokens are macOS-oriented —
review `--show` output).
