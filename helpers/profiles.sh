...
dots_write_runtime_policy() {
  ...
  # Home profile may request herdr; if herdr was not selected this run, fall back to tmux
  if [[ "${mux}" == "herdr" ]]; then
    if ! dots_array_contains herdr "${EFFECTIVE_WITH[@]+"${EFFECTIVE_WITH[@]}"}"; then
      if ! command -v herdr >/dev/null 2>&1; then
        echo "Note: runtime.multiplexer=herdr but herdr not selected/installed — writing tmux fallback"
        mux="tmux"
      fi
    fi
  fi
