...
members="${members% }"
want="hermes ollama drawthings opencode codex cursor fluidvoice"
...
for m in hermes ollama drawthings opencode codex cursor; do
...
if contains hermes "${got[@]+"${got[@]}"}" && contains ollama "${got[@]+"${got[@]}"}" && contains opencode "${got[@]+"${got[@]}"}"; then
	ok "linux ai keeps hermes/ollama/opencode"
else
...
dots_compute_effective_with
_ai_leak=0
for m in hermes ollama drawthings opencode codex cursor fluidvoice; do
