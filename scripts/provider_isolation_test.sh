...
# C
out="$(run_capture "DOTS_FORCE_OS=darwin DOTS_FORCE_DARWIN_MAJOR=15 \"${SETUP}\" --dry-run --with herdr,cursor")" || true
ok=1
...
# F — cursor is macOS-oriented; force Darwin for Linux CI
out="$(run_capture "DOTS_FORCE_OS=darwin DOTS_FORCE_DARWIN_MAJOR=15 \"${SETUP}\" --dry-run --with cursor,drawthings")" || true
ok=1
...
out="$(run_capture "DOTS_FORCE_OS=darwin DOTS_FORCE_DARWIN_MAJOR=15 \"${BOOT}\" --profile home --dry-run")" || true
ok=1
...
must_not "home" "${out}" 'Brewfile.fluidvoice' || ok=0
[[ "${ok}" -eq 1 ]] && { echo "OK: home profile"; pass=$((pass+1)); } || fail=$((fail+1))

out="$(run_capture "DOTS_FORCE_OS=darwin DOTS_FORCE_DARWIN_MAJOR=15 \"${SETUP}\" --dry-run --with cursor")" || true
ok=1
...
out="$(run_capture "DOTS_FORCE_OS=darwin DOTS_FORCE_DARWIN_MAJOR=15 \"${SETUP}\" --dry-run --with cursor,herdr")" || true
ok=1
...
# --- ai supergroup consent ---
out="$(run_capture "DOTS_FORCE_OS=darwin DOTS_FORCE_DARWIN_MAJOR=15 \"${SETUP}\" --dry-run --with ai")" || true
ok=1
must "ai" "${out}" 'Brewfile.hermes|hermes' || ok=0
must "ai" "${out}" 'Brewfile.ollama|ollama' || ok=0
must "ai" "${out}" 'Brewfile.cursor|Cursor Agent' || ok=0
must "ai" "${out}" 'Brewfile.fluidvoice|fluidvoice' || ok=0
must_not "ai" "${out}" 'herdr integration' || ok=0
[[ "${ok}" -eq 1 ]] && { echo "OK: --with ai expands + consents (no herdr)"; pass=$((pass+1)); } || fail=$((fail+1))

out="$(run_capture "DOTS_FORCE_OS=linux \"${SETUP}\" --dry-run --with ai")" || true
ok=1
must "ai-linux" "${out}" 'Brewfile.hermes|hermes' || ok=0
must_not "ai-linux" "${out}" 'Brewfile.fluidvoice' || ok=0
must_not "ai-linux" "${out}" 'Brewfile.cursor' || ok=0
[[ "${ok}" -eq 1 ]] && { echo "OK: --with ai on linux omits darwin-only"; pass=$((pass+1)); } || fail=$((fail+1))

out="$(run_capture "DOTS_FORCE_OS=linux \"${SETUP}\" --dry-run --with fluidvoice" || true)"
ok=1
must "fv-linux" "${out}" "supported only on macOS|not supported" || ok=0
[[ "${ok}" -eq 1 ]] && { echo "OK: explicit fluidvoice fails on linux"; pass=$((pass+1)); } || fail=$((fail+1))

# Plain work/server still no AI consent from presence
out="$(run_capture "\"${BOOT}\" --profile work --dry-run")" || true
ok=1
must_not "work2" "${out}" 'Brewfile.hermes' || ok=0
must_not "work2" "${out}" 'Brewfile.cursor' || ok=0
[[ "${ok}" -eq 1 ]] && { echo "OK: work still AI-free"; pass=$((pass+1)); } || fail=$((fail+1))

out="$(run_capture "\"${BOOT}\" --profile server --dry-run")" || true
ok=1
must_not "server" "${out}" 'Brewfile.hermes' || ok=0
must_not "server" "${out}" 'Brewfile.fluidvoice' || ok=0
must "server" "${out}" 'herdr' || ok=0
[[ "${ok}" -eq 1 ]] && { echo "OK: server herdr without AI clients"; pass=$((pass+1)); } || fail=$((fail+1))
...
if ! python3 -c 'import tomllib' 2>/dev/null && ! command -v python3.12 >/dev/null 2>&1 && ! command -v python3.11 >/dev/null 2>&1; then
  ...
  out="$(run_capture "DOTS_FORCE_OS=darwin DOTS_FORCE_DARWIN_MAJOR=15 \"${SETUP}\" --dry-run --with cursor,ai-skills" || true)"
  ok=1
